...
static void Task_HandlePokedexStartMenuInput(u8 taskId)
{
    ...
    else
    {
        ...
        else if (JOY_REPT(DPAD_UP) && sPokedexView->menuCursorPos != 0)
        {
            sPokedexView->menuCursorPos--;
            PlaySE(SE_SELECT);
        }
        else if (JOY_REPT(DPAD_DOWN) && sPokedexView->menuCursorPos < 3)
        {
            sPokedexView->menuCursorPos++;
            PlaySE(SE_SELECT);
        }
...
#define temp_dexNum     vars[2]
    ...
    case ORDER_ALPHABETICAL:
        for (i = 0; i < NATIONAL_DEX_COUNT; i++)
        {
            ...
            if ((!temp_isHoennDex || NationalToKantoDexNum(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN))
            {
    ...
    case ORDER_HEAVIEST:
        for (i = NATIONAL_DEX_COUNT - 1; i >= 0; i--)
        {
            ...
            if ((!temp_isHoennDex || NationalToKantoDexNum(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = temp_dexNum;
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].seen = TRUE;
...
static u8 CreateMonName(u16 num, u8 left, u8 top)
{
    ...
    num = NationalDexNumToSpecies(num);
    if (num)
...
static u32 CreatePokedexMonSprite(u16 num, s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_MONS_ON_SCREEN; i++)
    {
        if (sPokedexView->monSpriteIds[i] == 0xFFFF)
        {
            ...
            gSprites[spriteId].data[2] = NationalDexNumToSpecies(num);
...
static void CreateStatBars(struct PokedexListItem *dexMon)
{
    ...
    if (dexMon->owned) // Show filed bars
    {
        ...
        u32 species = NationalDexNumToSpecies(dexMon->dexNum);
...
static u16 NationalPokedexNumToSpeciesHGSS(u16 nationalNum)
{
    ...
    if (sPokedexView->formSpecies != 0)
        ...
        return NationalDexNumToSpecies(nationalNum);
}
...
static void Task_HandleStatsScreenInput(u8 taskId)
{
    ...
    //Change moves
    if (JOY_REPT(DPAD_UP) && sPokedexView->moveSelected > 0)
    {
    ...
    if (JOY_REPT(DPAD_DOWN) && sPokedexView->moveSelected < sPokedexView->movesTotal -1 )
    {
...
static void GetSeenFlagTargetSpecies(void)
{
    ...
    for (i = 0; i < sPokedexView->sEvoScreenData.numAllEvolutions; i++)
    {
        ...
        if (GetSetPokedexFlag(SpeciesToNationalDexNum(species), FLAG_GET_SEEN))
        {
...
static void Task_HandleEvolutionScreenInput(u8 taskId)
{
    ...
    if (sPokedexView->sEvoScreenData.numAllEvolutions > 0 && sPokedexView->sEvoScreenData.numSeen > 0)
    {
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            u16 dexNum          = SpeciesToNationalDexNum(targetSpecies);
...
{
    bool32 seen = GetSetPokedexFlag(SpeciesToNationalDexNum(targetSpecies), FLAG_GET_SEEN);
...
{
    bool8 owned = GetSetPokedexFlag(SpeciesToNationalDexNum(targetSpecies), FLAG_GET_CAUGHT);
    if (owned)
    ...
    else
    {
        ...
        PrintInfoScreenTextSmall(gText_PokeSum_OneHyphen, x+1, y-1);
    }
}
...
{
    bool8 seen = GetSetPokedexFlag(SpeciesToNationalDexNum(preSpecies), FLAG_GET_SEEN);
...
static void Task_HandleFormsScreenInput(u8 taskId)
{
    ...
    else //inSubmenu
    {
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            u16 formSpecies = GetFormSpeciesId(NationalDexNumToSpecies(sPokedexListItem->dexNum), formId);
            if (sPokedexView->isSearchResults && sPokedexView->originalSearchSelectionNum == 0)
                sPokedexView->originalSearchSelectionNum = sPokedexListItem->dexNum;
...
static void Task_HandleSearchResultsStartMenuInput(u8 taskId)
{
    ...
    else
    {
        ...
        else if (JOY_REPT(DPAD_UP) && sPokedexView->menuCursorPos)
        {
            sPokedexView->menuCursorPos--;
            PlaySE(SE_SELECT);
        }
        else if (JOY_REPT(DPAD_DOWN) && sPokedexView->menuCursorPos < 4)
        {
            sPokedexView->menuCursorPos++;
            PlaySE(SE_SELECT);
        }
...
static int DoPokedexSearch(u8 dexMode, u8 order, u8 abcGroup, u8 bodyColor, u8 type1, u8 type2)
{
    ...
    if (abcGroup != 0xFF)
    {
        for (i = 0, resultsCount = 0; i < sPokedexView->pokemonListCount; i++)
        {
            ...
            species = NationalDexNumToSpecies(sPokedexView->pokedexList[i].dexNum);
